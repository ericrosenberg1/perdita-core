...
class Perdita_MCP {
	...
	public static function rate_limit_key( $prefix, $ip ) {
		...
		$ip = (string) $ip;
		if ( false !== strpos( $ip, ':' ) && false !== ( $packed = @inet_pton( $ip ) ) && 16 === strlen( $packed ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged, Generic.CodeAnalysis.AssignmentInCondition.Found -- invalid input simply keeps the raw string.
